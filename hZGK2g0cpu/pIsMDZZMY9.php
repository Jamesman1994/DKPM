...
if ($dkpm_conn->connect_errno) {
    ...
    $access_token = hex2bin($data->WI854oIvuA);
    ...
    if ($stmt_check_account->num_rows) {
        $stmt_get_lock_time = $dkpm_conn->prepare("SELECT AES_DECRYPT(LOCK_TIME, '".$key."') FROM dkpm.user_account WHERE AES_DECRYPT(USER_ID , '".$key."') = ?");
        $stmt_get_lock_time->bind_param("s", $user_id);
        $stmt_get_lock_time->execute();
        $stmt_get_lock_time->bind_result($lock_time);
        $stmt_get_lock_time->store_result();
        $stmt_get_lock_time->fetch();
        $stmt_get_lock_time->close();

        http_response_code(200);
        $validation_array = array("error" => false, "NndiqN27J5" => $lock_time);
        echo json_encode($validation_array);
    } else {
        http_response_code(401);
