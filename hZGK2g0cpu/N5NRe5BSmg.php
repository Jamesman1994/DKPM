...
if ($dkpm_conn->connect_errno) {
    ...
    $access_token = hex2bin($data->zqzFY26dQR);
    ...
    if ($stmt_check_account->num_rows) {
        $stmt_logout = $dkpm_conn->prepare("UPDATE dkpm.user_account set ONE_TIME_TOKEN = AES_ENCRYPT('', '".$key."') WHERE AES_DECRYPT(ONE_TIME_TOKEN, '".$key."') = ?");
        $stmt_logout->bind_param("s", $decrypted_token);
        $stmt_logout->execute();
    
        if ($stmt_logout->execute()) {
            http_response_code(200);
            $validation_array = array("error" => false, "message" => "fail");
            echo json_encode($validation_array);
        } else {
            http_response_code(401);
