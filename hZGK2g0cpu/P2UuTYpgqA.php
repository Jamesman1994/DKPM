...
if ($dkpm_conn->connect_errno) {
    ...
    $access_token = hex2bin($data->VSD59Ay7R2);
    $record_id = hex2bin($data->j8pr2gyl9s);
    ...
    if ($stmt_check_account->num_rows) {
        $stmt_detail = $dkpm_conn->prepare("SELECT AES_DECRYPT(uav.RECORD_ID, '".$vault_key."'), AES_DECRYPT(uav.ITEM_NAME, '".$vault_key."'), AES_DECRYPT(uav.ITEM_ICON, '".$vault_key."'), AES_DECRYPT(uav.USER_NAME, '".$vault_key."'), AES_DECRYPT(uav.PASSWORD, '".$vault_key."'), AES_DECRYPT(uav.DOMAIN, '".$vault_key."'), AES_DECRYPT(uav.URL, '".$vault_key."'), AES_DECRYPT(uav.REMARKS, '".$vault_key."'), uav.RECORD_TIME, AES_DECRYPT(ua.USER_PASSWORD, '".$key."') FROM dkpm.user_account_vault uav LEFT JOIN dkpm.user_account ua on AES_DECRYPT(uav.USER_ID, '".$vault_key."') = AES_DECRYPT(ua.USER_ID, '".$key."') WHERE AES_DECRYPT(uav.RECORD_ID, '".$vault_key."') = ?");
        $stmt_detail->bind_param("s", $record_id);
        $stmt_detail->execute();
        $stmt_detail->bind_result($record_id, $item_name, $item_icon, $user_name, $password, $domain, $url, $remarks, $record_time, $user_password);
        $stmt_detail->store_result();
        $stmt_detail->fetch();

        if ($stmt_detail->num_rows) {
            $decrypted_pw = secondEn::decrypt($password, $user_password);
            $item_array = array('u86wjzd0b4' => $record_id, 'item_name' => $item_name, 'item_icon' => $item_icon, 'user_name' => $user_name, 'password' => $decrypted_pw, 'domain' => $domain, 'url' => $url, 'remarks' => $remarks);
